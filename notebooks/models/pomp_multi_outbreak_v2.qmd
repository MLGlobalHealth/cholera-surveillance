...
IDPS values are not available for the whole time range (i.e. they only start from around March 2018). For now we just assuming that there were no population movements up to that time for simplicity. Another approach would be to limit the investigation to the time period 2018-2024. We also note that these numbers are clearly to large to be realistic as almost all the cit
...
df_reports <- (
  ...
  %>% mutate(
    ...
    cov = cov - min(cov),
    idps = replace_na(idps, 0) / 7, # We bring this back to daily numbers to match the time step
...
df_dem <- tibble(
  ...
  birthrate = c(43.5, 43.4, 43, 42.7, 42.3, 42, 41.7, 41.3, 41, 40.7) * 1.05 / (1000) # rates are adjusted to better reflect data that Andrew shared
)
```
...
m_rbi_1 <- "
  model m_rbi_1 {
    ...
    param s_0, e_0, a_0, i_0, r_0
    param Beta, eps_A, f_ai, mu_rs, tau, rho
    param sigmaSE
    state S, E, A, I, R, C, N, W
...
dmeas_2 <- Csnippet("
  ...
  if (reports > 0.0) {
    lik = pnorm(reports+0.5,mean_reports,sd_reports,1,0)
      - pnorm(reports-0.5,mean_reports,sd_reports,1,0);
  }
  else{
    lik = pnorm(reports+0.5,mean_reports,sd_reports,1,0);
  }
...
```{r}
start <- Sys.time()
bayes_start_model_2 <- m_global_2[[which.min(loglik_global_all_2[,1])]] 
m_bayes_2 <- (
  bayes_start_model_2
    %>% pomp(
      # t0 = df_model$date[1],
...
pairs(
  ...
  data=subset(model_params_3, log_lik > max(log_lik) - 70)
)
```
...
```{r}
model_output_filename_5 <- file.path(output_dir, "uvira_multioutbreak_cov_idps_notsym.csv")
```
...
# With Covariate Modulation + IDPS + Hospitalized (scaled)
...
rproc_6 <- Csnippet("
  ...
  if (idps < 0) {
    if (N > H) {
      ...
      idps_scaled = floor(fmin((-idps * b_idps), N-H));
      s = (N-H) / (S + E + A + I + R);
      rate_idps[0] = s*S/(N-H);
      rate_idps[1] = s*E/(N-H);
      rate_idps[2] = s*A/(N-H);
      ...
      R -= fmin(trans_idps[4], R);
    }
  }
  if (idps > 0) {
    //idps_scaled = floor(fmin((idps * b_idps)/5, (N-H)/10));
    idps_scaled = floor(fmin(idps * b_idps, N-H));
    trans_idps[0] = nearbyint(idps_scaled/5);
    trans_idps[1] = nearbyint(idps_scaled/5);
    trans_idps[2] = nearbyint(idps_scaled/5);
    trans_idps[3] = nearbyint(idps_scaled/5);
    trans_idps[4] = nearbyint(idps_scaled/5);
    S += trans_idps[0];
    E += trans_idps[1];
    A += trans_idps[2];
    I += trans_idps[3];
...
sims_global_best_ll_6 <- (
  m_global_6[[which(rank(-loglik_global_all_6[,1]) == 1)]]
  %>% simulate(
    nsim = 100,
    format = "data.frame",
    include.data = TRUE
...
# With Covariate Modulation + IDPS + Hospitalized (not scaled)

```{r}
model_output_filename_10 <- file.path(output_dir, "uvira_multioutbreak_cov_idps_hosp_ns.csv")
```
...
paramnames_10 <- c(rp_names_log_10, rp_names_logit_10, ivp_names_10, names(fixed_params_10))
accumvars_10 <- c(
  "C",
  "W"
)
partrans_10 <- parameter_trans(
  log=rp_names_log_10,
  logit=rp_names_logit_10,
  barycentric=ivp_names_10
)
...
m_10 <- (
  ...
  %>% pomp(
    ...
    rmeasure = rmeas_10,
    dmeasure = dmeas_10,
    obsnames = obsnames_10,
    covar = covar_10,
    covarnames = covarnames_10,
    accumvars = accumvars_10,
    statenames = statenames_10,
    paramnames = paramnames_10,
    partrans = partrans_10,
    params = init_params_10
  )
)
```
...
```{r}
pf_sim_10 <- pfilter(m_10, Np = 1000)
lk_sim_10 <- logLik(pf_sim_10)
lk_sim_10
```
...
m_local_10 <- foreach (
  ...
  init_params_10[rp_names_log_10] <- rlnorm(
    n=length(rp_names_log_10), 
    meanlog=log(init_params_10[rp_names_log_10]),
    sdlog=1
  )
  init_params_10[c(rp_names_logit_10, ivp_names_10)] <- runif(
    n=length(c(rp_names_logit_10, ivp_names_10)),
    min=0.01,
    max=0.99
  )
  (
...
```{r}
m_local_mle_10 <- m_local_10[[which.max(loglik_local_all_10[,1])]]
params_local_10 <- coef(m_local_mle_10)
loglik_local_best_10 <- logmeanexp(
  replicate(
    n = Nreps_eval,
    logLik(pfilter(m_local_mle_10, Np = 1000))
  ),
  se = TRUE
)
```
...
```{r}
loglik_local_best_10
```

### Collect Results

```{r}
df_results_local_10 <- data.frame(
  t(sapply(m_local_10, coef)),
  log_lik=loglik_local_all_10[,1],
  log_lik_se=loglik_local_all_10[,2]
)
```
...
if (run_level > 0) {
  ...
  col.names <- TRUE
  row.names <- FALSE
  if (file.exists(model_output_filename_10)) {
    append <- TRUE
    col.names <- FALSE
  }
  (
...
```{r}
model_params_10 <- read.table(
  model_output_filename_10,
  row.names = NULL,
  header = TRUE
)
```
...
```{r}
summary(model_params_10$log_lik, digits = 5)
```
...
(
  ...
  %>% traces()
  %>% melt()
  %>% filter(!name %in% c(names(fixed_params_10), ivp_names_10))
  %>% ggplot(
    aes(
      x=iteration,
      y=value,
...
global_guesses_10 <- as.data.frame(cbind(
  s_0 = s_0_guesses,
  e_0 = e_0_guesses,
  a_0 = a_0_guesses,
  i_0 = i_0_guesses,
...
```{r}
start <- Sys.time()
m_global_10 <- foreach (guess=iter(global_guesses_10, "row"), .combine=c) %dopar% {
  (
    m_local_mle_10
    %>% mif2(Nmif=Nmif, params=c(fixed_params_10, unlist(guess)))
    %>% mif2(Nmif=Nmif, cooling.fraction.50 = 0.1)
  )
}
end <- Sys.time()
...
loglik_global_all_10 <- (
  foreach(
    ...
    logmeanexp(
      replicate(
        Nreps_eval,
        logLik(pfilter(m_10, params=coef(m_global_10[[i]]), Np=1000))
      ),
      se=TRUE
    )
  }
...
```{r}
df_results_global_10 <- (
  data.frame(
    t(sapply(m_global_10, coef)),
    log_lik=loglik_global_all_10[,1],
    log_lik_se=loglik_global_all_10[,2]
  )
  %>% select(
    names(df_results_local_10)
  )
)
```
...
```{r}
sims_global_best_ll_10 <- (
  m_global_10[[which(rank(-loglik_global_all_10[,1]) == 1)]]
  %>% simulate(
    nsim = 100,
    format = "data.frame",
    include.data = TRUE
...
sims_global_best_sd_10 <- (
  m_global_10[[which.min(loglik_global_all_10[,2])]]
  %>% simulate(
    nsim = 100,
    format = "data.frame",
    include.data = TRUE
...
p1 <- (
  ...
  + geom_line(
      data = sims_global_best_ll_agg_10,
      mapping = aes(x = date, y = reports_median), 
      color = "cornflowerblue", 
      linetype = "solid",
      linewidth = 1
...
p2 <- (
  ...
  + geom_line(
      data = sims_global_best_sd_agg_10,
      mapping = aes(x = date, y = reports_median), 
      color = "cornflowerblue", 
      linetype = "solid",
      linewidth = 1
...
```{r}
coef(m_global_10[[which(rank(-loglik_global_all_10[,1]) == 1)]])
```
